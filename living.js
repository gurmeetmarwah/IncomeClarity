...
function initLivingHub() {
  const hub = document.querySelector(".living-hub");
  if (!hub) {
    return;
  }

  const groups = hub.querySelectorAll(".living-hub-group");
  const pills = hub.querySelectorAll(".living-hub-jump-pill");

  function setActivePill(id) {
    pills.forEach((pill) => {
      const target = pill.getAttribute("data-living-hub-target");
      pill.classList.toggle("is-active", target === id);
    });
  }

  pills.forEach((pill) => {
    pill.addEventListener("click", (event) => {
      const id = pill.getAttribute("data-living-hub-target");
  ...
  groups.forEach((group) => {
    group.addEventListener("toggle", () => {
      ...
      setActivePill(group.id);
    });
  });

  const openGroup = hub.querySelector(".living-hub-group[open]");
  if (openGroup) {
    setActivePill(openGroup.id);
  }
}

if (document.readyState === "loading") {
  document.addEventListener("DOMContentLoaded", initLivingHub);
} else {
  initLivingHub();
}
